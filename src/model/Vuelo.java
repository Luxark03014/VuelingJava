...
public class Vuelo {
    ...
    public static void añadirVuelo(Scanner scanner, List<Vuelo> vuelosDisponibles) throws CompaniaNoEncontradaException {
        System.out.println("\n--- Añadir un vuelo ---");
        ...
        for (int i = 0; i < Companias.values().length; i++) {
            ...
            scanner.nextLine();
            if (opcionCompania < 1 || opcionCompania > Companias.values().length) {
                // Lanzar excepción si el número está fuera del rango válido
                throw new CompaniaNoEncontradaException("Error: La compañía seleccionada no existe. Introduzca un número válido.");
            }
